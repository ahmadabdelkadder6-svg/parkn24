...
function GarageCard({
  ...
  const btnShadow = (() => {
    if (garage.availableSpots === 0) return 'none';
    if (hasActiveSession) return '0 6px 24px rgba(0,204,102,0.35)';
    if (hasIncomingCar) return '0 6px 24px rgba(0,153,221,0.35)';
    return '0 6px 24px rgba(0,102,255,0.35)';
  })();

  const btnLabel = (() => {
    if (garage.availableSpots === 0) return 'ممتلئ - لا يمكن الحجز';

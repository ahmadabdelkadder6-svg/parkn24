...
import {
  calculateDistance, distanceToMinutes, classifyDistance, formatDuration,
} from '../utils/distance';
import TopUpWalletModal from './TopUpWalletModal';
import toast from 'react-hot-toast';

interface GarageWithDistance extends Garage {
  distance: number; minutes: number; classification: 'nearby' | 'far';
}

const normalizePlateForCompare = (plate?: string): string => {
  if (!plate) return '';
  return plate.trim().replace(/[٠-٩]/g,(d)=>String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))).replace(/[۰-۹]/g,(d)=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d))).replace(/\s+/g,' ').toUpperCase();
};

export default function GarageListScreen() {
  const { garages, setSelectedGarageId, setScreen, currentUser, sessions, incomingCars, offers, addIncomingCar } = useStore();

  const [search, setSearch] = useState('');
  const [showNearbyOnly, setShowNearbyOnly] = useState(false);
  const [showTopUp, setShowTopUp] = useState(false);
  const [userLocation, setUserLocation] = useState<{lat:number;lng:number}>({lat:30.0444,lng:31.2357});
  const [locationLoading, setLocationLoading] = useState(false);

  const normalizedUserPlate = normalizePlateForCompare(currentUser?.carPlate);
  const hasCompletedSession = sessions.some(s=>normalizePlateForCompare(s.carPlate)===normalizedUserPlate&&s.status==='completed');
  const activeSession = sessions.find(s=>normalizePlateForCompare(s.carPlate)===normalizedUserPlate&&s.status==='active');
  const myIncomingCar = incomingCars.find(c=>normalizePlateForCompare(c.carPlate)===normalizedUserPlate&&c.status==='coming');

  const getUserLocation = () => {
    setLocationLoading(true);
    if('geolocation' in navigator){navigator.geolocation.getCurrentPosition(p=>{setUserLocation({lat:p.coords.latitude,lng:p.coords.longitude});setLocationLoading(false);},()=>setLocationLoading(false),{enableHighAccuracy:true,timeout:10000});}
  ...
  const garagesWithDistance: GarageWithDistance[] = useMemo(()=>{
    return garages.map(garage=>{const distance=calculateDistance(userLocation.lat,userLocation.lng,garage.lat,garage.lng);const minutes=distanceToMinutes(distance);return{...garage,distance,minutes,classification:classifyDistance(minutes)};}).sort((a,b)=>a.minutes-b.minutes);
  },[garages,userLocation]);

  const filteredGarages = useMemo(()=>{let filtered=garagesWithDistance;if(search)filtered=filtered.filter(g=>g.name.includes(search)||g.location.includes(search));if(showNearbyOnly)filtered=filtered.filter(g=>g.classification==='nearby');return filtered;},[garagesWithDistance,search,showNearbyOnly]);
  ...
  const handleDirectBooking = (garage: GarageWithDistance) => {
    ...
    addIncomingCar({garageId:garage.id,carPlate:currentUser.carPlate,customerName:currentUser.name,customerPhone:currentUser.phone,agreedPrice:garage.basePrice,estimatedArrival:Math.max(3,garage.minutes)});
    ...
    <div className="h-full flex flex-col" style={{background:'#EBF2FF',color:'#0A1628'}}>
      ...
      <div className="px-4 pt-12 pb-3" style={{background:'#ffffff'}}>
        ...
        <div style={{background:'linear-gradient(135deg, #0066FF 0%, #4D00FF 100%)',borderRadius:24,padding:'20px 18px',marginBottom:14,boxShadow:'0 8px 32px rgba(0,102,255,0.35)',color:'#ffffff'}}>
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <button onClick={()=>setShowTopUp(true)} className="flex items-center gap-1 font-black active:scale-95 transition-transform"
                style={{background:'rgba(255,255,255,0.2)',backdropFilter:'blur(10px)',borderRadius:14,padding:'10px 16px',fontSize:12}}>
                <Plus size={14} /> شحن
              </button>
            </div>
            <div className="font-black" style={{background:'rgba(255,255,255,0.15)',backdropFilter:'blur(10px)',borderRadius:14,padding:'10px 14px',fontSize:12}}>
        ...
        {activeSession && (
          <motion.button initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}}
            ...
            style={{background:'linear-gradient(135deg, #00CC66 0%, #00AA55 100%)',borderRadius:20,padding:'16px 18px',color:'#ffffff',boxShadow:'0 0 30px rgba(0,204,102,0.4), 0 8px 24px rgba(0,204,102,0.25)'}}>
        ...
        {!activeSession && myIncomingCar && (
          <motion.button initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}}
            ...
            className="w-full mb-3 flex items-center justify-between active:scale-[0.97] transition-all"
            style={{background:'linear-gradient(135deg, #0099DD 0%, #0077BB 100%)',borderRadius:20,padding:'16px 18px',color:'#ffffff',boxShadow:'0 0 30px rgba(0,153,221,0.4), 0 8px 24px rgba(0,153,221,0.25)'}}>
            <div className="flex items-center gap-2">
              <motion.span animate={{scale:[1,1.3,1]}} transition={{repeat:Infinity,duration:1.5}} className="w-3 h-3 rounded-full bg-white" />
        ...
        <div className="flex gap-2 mb-2">
          <div className="relative flex-1">
            ...
            <input className="w-full font-bold outline-none text-sm" style={{background:'#F0F4FF',border:'2px solid #D0DCFF',padding:'14px 40px 14px 14px',borderRadius:18,color:'#0A1628'}}
              placeholder="ابحث عن جراج..." value={search} onChange={e=>setSearch(e.target.value)} />
          </div>
          <button onClick={getUserLocation} disabled={locationLoading} className="active:scale-90 transition-all"
            style={{background:locationLoading?'#E2E8F0':'#0066FF',color:locationLoading?'#94a3b8':'#fff',borderRadius:18,padding:'0 16px',boxShadow:locationLoading?'none':'0 4px 16px rgba(0,102,255,0.3)'}}>
            <Locate size={20} className={locationLoading?'animate-spin':''} />
          </button>
          <button onClick={()=>setShowNearbyOnly(!showNearbyOnly)} className="font-black text-xs active:scale-90 transition-all whitespace-nowrap flex items-center gap-1"
            style={{background:showNearbyOnly?'#0066FF':'#F0F4FF',color:showNearbyOnly?'#fff':'#64748b',borderRadius:18,padding:'0 16px',border:showNearbyOnly?'none':'2px solid #D0DCFF',boxShadow:showNearbyOnly?'0 4px 16px rgba(0,102,255,0.3)':'none'}}>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-3 pb-6">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          {hasCompletedSession && (
            <button onClick={()=>setScreen('lastSession')} className="flex items-center gap-2 active:scale-[0.97] transition-all"
              style={{background:'#ffffff',border:'1.5px solid #D0DCFF',borderRadius:16,padding:'10px 10px',boxShadow:'0 2px 8px rgba(0,102,255,0.06)'}}>
          ...
          <button onClick={()=>setScreen('chat')}
            ...
            style={{background:'#ffffff',border:'1.5px solid #E0D6FF',borderRadius:16,padding:'10px 10px',boxShadow:'0 2px 8px rgba(124,58,237,0.06)'}}>
        ...
        {nearbyGarages.length>0&&(
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3 justify-end">
              <span className="font-black" style={{background:'#00CC66',color:'#fff',fontSize:11,padding:'3px 12px',borderRadius:20,boxShadow:'0 2px 8px rgba(0,204,102,0.3)'}}>{nearbyGarages.length}</span>
        ...
        {farGarages.length>0&&!showNearbyOnly&&(
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3 justify-end">
              <span className="font-black" style={{background:'#FF8800',color:'#fff',fontSize:11,padding:'3px 12px',borderRadius:20,boxShadow:'0 2px 8px rgba(255,136,0,0.3)'}}>{farGarages.length}</span>
...
function GarageCard({garage,index,onSelect,isNearby,isClosest,hasActiveSession,hasIncomingCar}:{
  garage:GarageWithDistance;index:number;onSelect:()=>void;isNearby:boolean;isClosest?:boolean;hasActiveSession?:boolean;hasIncomingCar?:boolean;
}) {
  ...
  return (
    <motion.div initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{delay:index*0.05}}
      ...
      style={{background:'#ffffff',border:`2.5px solid ${borderColor}`,borderRadius:24,padding:'18px 16px',boxShadow:`0 4px 20px ${glowColor}, 0 2px 8px rgba(0,0,0,0.04)`}}>

      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1 font-black" style={{background:'#FF9500',color:'#fff',fontSize:11,padding:'4px 10px',borderRadius:12,boxShadow:'0 2px 8px rgba(255,149,0,0.3)'}}>
          ...
          {!isBusy&&isClosest&&garage.availableSpots>0&&(<span className="font-black" style={{background:'#0066FF',color:'#fff',fontSize:10,padding:'4px 10px',borderRadius:12,boxShadow:'0 0 12px rgba(0,102,255,0.3)'}}>📍 الأقرب</span>)}
      ...
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2 font-black"
          style={{background:isNearby?'#00CC66':'#FF8800',color:'#fff',borderRadius:16,padding:'10px 16px',fontSize:14,boxShadow:isNearby?'0 4px 16px rgba(0,204,102,0.3)':'0 4px 16px rgba(255,136,0,0.3)'}}>
      ...
      <button disabled={garage.availableSpots===0} className="w-full font-black flex items-center justify-center gap-2 active:scale-95 transition-all"
        style={{
          background:garage.availableSpots===0?'#E2E8F0':hasActiveSession?'linear-gradient(135deg, #00CC66 0%, #00AA55 100%)':hasIncomingCar?'linear-gradient(135deg, #0099DD 0%, #0077BB 100%)':isClosest?'linear-gradient(135deg, #0066FF 0%, #0044DD 100%)':isNearby?'linear-gradient(135deg, #00CC66 0%, #00AA55 100%)':'linear-gradient(135deg, #0066FF 0%, #4D00FF 100%)',
          color:garage.availableSpots===0?'#94a3b8':'#ffffff',borderRadius:18,padding:'16px 0',fontSize:14,
          boxShadow:garage.availableSpots===0?'none':hasActiveSession?'0 6px 24px rgba(0,204,102,0.35)':hasIncomingCar?'0 6px 24px rgba(0,153,221,0.35)':'0 6px 24px rgba(0,102,255,0.35)'}}>
        <Car size={18} />
        {garage.availableSpots===0?'ممتلئ - لا يمكن الحجز':hasActiveSession?'⚡ عرض جلستك النشطة':hasIncomingCar?'📍 عرض حجزك النشط':isClosest?'🅿️ احجز الأقرب إليك':isNearby?'🅿️ احجز - قريب منك':'🅿️ احجز مكان'}
      </button>
    </motion.div>
  );
}
...
import {
  ...
  Plus, Receipt, MessageCircle, DollarSign, Zap, Gift,
} from 'lucide-react';
...
export default function GarageListScreen() {
  const {
    garages, setSelectedGarageId, setScreen, currentUser, sessions,
    incomingCars, offers, addIncomingCar, fetchAll, loyaltyStatus,
  } = useStore();
  ...
  useEffect(() => {
    if (currentUser?.phone) {
      useStore.getState().fetchLoyaltyStatus();
    }
  }, [currentUser?.phone]);
  ...
  useEffect(() => {
    ...
    const isMyRow = (row: any) => {
      ...
      return plate === normalizedUserPlate || (!!currentUser?.phone && phone === currentUser.phone);
    };

    const channel = supabase
      .channel(`customer-list-${normalizedUserPlate}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'sessions' },
        async (p) => { if (isMyRow(p.new) || isMyRow(p.old)) await refetch(); })
...
addIncomingCar({
      ...
      isFreeSession: loyaltyStatus?.isNextFree || false,
    } as any);
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {loyaltyStatus.isNextFree ? <Gift size={20} style={{ color: '#0F172A' }} /> : <span className="text-lg">🅿️</span>}
                <div>
                  <div className="font-black" style={{ fontSize: loyaltyStatus.isNextFree ? 13 : 11, color: loyaltyStatus.isNextFree ? '#0F172A' : '#94A3B8' }}>
                    {loyaltyStatus.isNextFree ? '🎉 ركنتك الجاية مجانية!' : 'برنامج الولاء'}
                  </div>
                  <div className="font-bold" style={{ fontSize: 10, color: loyaltyStatus.isNextFree ? '#1E293B' : '#64748B' }}>
                    {loyaltyStatus.isNextFree
                      ? 'حد أقصى 2 ساعة مجانية 🎁'
                      : `${loyaltyStatus.paidSessions}/5 ركنات - باقي ${loyaltyStatus.remainingForFree} للركنة المجانية`}
                  </div>
                </div>
              </div>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} style={{
                    ...
                    border: loyaltyStatus.isNextFree ? '1px solid rgba(15,23,42,0.5)' : '1px solid #475569',
                  }} />
                ))}
              </div>
            </div>
...
function GarageCard({ garage, index, onSelect, isNearby, isClosest, hasActiveSession, hasIncomingCar }: GarageCardProps) {
  ...
  const btnBg = (() => {
    ...
    if (hasActiveSession) return 'linear-gradient(135deg, #00CC66, #00AA55)';
    if (hasIncomingCar) return 'linear-gradient(135deg, #0099DD, #0077BB)';
    if (isClosest) return 'linear-gradient(135deg, #0066FF, #0044DD)';
    if (isNearby) return 'linear-gradient(135deg, #00CC66, #00AA55)';
    return 'linear-gradient(135deg, #0066FF, #4D00FF)';
  })();

  const btnLabel = (() => {
    if (garage.availableSpots === 0) return 'ممتلئ';
    if (hasActiveSession) return '⚡ عرض جلستك النشطة';
    if (hasIncomingCar) return '📍 عرض حجزك النشط';
    if (isClosest) return '🅿️ احجز الأقرب إليك';
    if (isNearby) return '🅿️ احجز - قريب منك';
    return '🅿️ احجز مكان';
  })();

...
import { motion, AnimatePresence } from 'framer-motion'; // 🚀 تم إضافة الاستيراد المفقود لحل الشاشة البيضاء فوراً
import {
...
export default function AdminDashboard() {
  ...
  const [activeAccordionGarageId, setActiveAccordionGarageId] = useState<string | null>(null); // ✅ التحكم في فتح وغلق أكورديون الجراجات النشطة
  ...
  const handleConfirmSettlement = async (garageId: string) => {
    ...
    try {
      ...
      toast.dismiss(loadingToast);
      console.error('Block cleanup failed:', error);
  ...
  return (
    <div className="h-full overflow-y-auto pt-16" style={{ background: '#EBF2FF', color: '#0A1628', padding: 16 }}>
      ...
      {/* 📊 جدول التسويات النشطة للجراجات (تصميم مدمج وقابل للطي - Accordion) */}
      <div className="mb-6 space-y-2">
        <div className="flex items-center justify-between mb-3 justify-end">
        ...
        {commissionStats.perGarage.length === 0 ? (
          <div className="text-center py-8 bg-white rounded-2xl border-2 border-dashed border-[#D0DCFF]">
            <span style={{ fontSize: 36, display: 'block', marginBottom: 8 }}>⚖️</span>
            <p className="font-black text-slate-500 text-xs">جميع حسابات الجراجات متزنة ومقفلة بالكامل!</p>
          </div>
        ) : (
          commissionStats.perGarage.map(g => {
            ...
            return (
              ...
              >
                {/* رأس السطر القابل للضغط للفتح والغلق */}
                <div 
                  onClick={() => {
                    setActiveAccordionGarageId(isExpanded ? null : g.id);
                    setConfirmSettlementGarageId(null); // إلغاء أي تأكيد معلق عند الطي
                  }}
                  className="flex justify-between items-center p-4 cursor-pointer hover:bg-slate-50/50 active:scale-[0.99] transition-all"
                >
                  {/* يسار السطر: قيمة التسوية مع شارة الاتجاه */}
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-400 font-bold" style={{ fontSize: 11 }}>{isExpanded ? '▲' : '▼'}</span>
                  </div>

                  {/* يمين السطر: اسم الجراج وعدد الجلسات */}
                  <div className="text-right">
                    <div className="font-black" style={{ fontSize: 14, color: '#0A1628' }}>{g.name}</div>
                    <div className="text-[10px] text-slate-400 font-bold mt-0.5">{g.totalCount} جلسة معلقة تسويتها</div>
                  </div>
                </div>

                {/* 📂 محتوى تفاصيل التسوية (يظهر فقط عند التمدد والفتح) */}
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div 
                      ...
                      className="px-4 pb-4 border-t border-slate-100 bg-slate-50/40"
                    >
                      <div className="grid grid-cols-3 gap-2 my-3">
                        <div className="text-center" style={{ background: '#F8FAFF', borderRadius: 12, padding: '8px 4px', border: '1px solid #D0DCFF' }}>
                          ...
                          <div className="font-black font-mono mt-0.5" style={{ fontSize: 13, color: '#0066FF' }}>{g.walletRevenue.toFixed(0)}</div>
                        </div>
                        <div className="text-center" style={{ background: '#FFF8F0', borderRadius: 12, padding: '8px 4px', border: '1px solid #FFD180' }}>
                          ...
                          <div className="font-black font-mono mt-0.5" style={{ fontSize: 13, color: '#FF9500' }}>{g.commission.toFixed(0)}</div>
                        </div>
                        <div className="text-center" style={{ background: adminOwesGarage ? '#EBFDF2' : '#FFF3F3', borderRadius: 12, padding: '8px 4px', border: `1px solid ${adminOwesGarage ? '#00CC66' : '#FF3333'}` }}>
                          ...
                          <div className="font-black font-mono mt-0.5" style={{ fontSize: 15, color: adminOwesGarage ? '#00AA44' : '#CC0000' }}>{absSettlement}</div>
                        </div>
                      </div>
                      ...
                      {isConfirming ? (
                        <div className="p-3 bg-amber-50 border-2 border-amber-200 rounded-2xl">
                          <p className="font-black text-center mb-1.5" style={{ fontSize: 11, color: '#3D1F00' }}>
                            ⚠️ هل تم فعلياً {adminOwesGarage ? 'تحويل' : 'استلام'} <span className="text-sm" style={{ color: adminOwesGarage ? '#00AA44' : '#CC0000', fontWeight: 900 }}>{absSettlement} ج.م</span>؟
                          </p>
                          <p className="text-center mb-3" style={{ fontSize: 9, color: '#7B8CA6' }}>
                            عند التأكيد سيتم قفل {g.totalCount} جلسة نهائياً ولن تظهر في الحسابات الحالية مرة أخرى.
                          </p>
                          <div className="flex gap-2">
                            <button 
                              onClick={() => {
                                handleConfirmSettlement(g.id);
                                setActiveAccordionGarageId(null); // غلق السطر بعد التأكيد
                              }} 
                  ...
                  {isEditingComm && (
                    <div className="flex items-center justify-center gap-2 mt-2">
                      <button onClick={() => setEditCommissionRate(r => Math.max(0, r - 1))} className="active:scale-90"
                        style={{ background: '#FF3333', color: '#fff', width: 30, height: 30, borderRadius: 9, display: 'flex', alignItems: 'center', justify_center: 'center' }}>
                        <Minus size={14} />
                      </button>
                      ...
                      <button onClick={() => setEditCommissionRate(r => Math.min(100, r + 1))} className="active:scale-90"
                        style={{ background: '#00CC66', color: '#fff', width: 30, height: 30, borderRadius: 9, display: 'flex', alignItems: 'center', justify_center: 'center' }}>
                        <Plus size={14} />
                      </button>
                    </div>
                  )}
...
export default function AdminDashboard() {
  ...
  const [processingSettlement, setProcessingSettlement] = useState(false);
  const [showArchive, setShowArchive] = useState(false);
  const [archiveSearch, setArchiveSearch] = useState(''); // 🚀 تصفية الأرشيف بالاسم
  const [visibleSettlements, setVisibleSettlements] = useState(4); // 🚀 تمديد الأرشيف
  const [activeAccordionGarageId, setActiveAccordionGarageId] = useState<string | null>(null); // 🚀 الأكورديون للنشط
  ...
  return (
    <div className="h-full overflow-y-auto pt-16" style={{ background: '#EBF2FF', color: '#0A1628', padding: 16 }}>
      ...
      {/* 📂 أرشيف التسويات المُقفلة (ذكي ومدمج وقابل للتمدد مع بحث داخلي وبخط أسود عريض واضح) */}
      <div className="mb-5">
        <button 
          onClick={() => { setShowArchive(!showArchive); setVisibleSettlements(4); setArchiveSearch(''); }} 
          className="w-full font-black flex items-center justify-between active:scale-95 transition-all"
            ...
            {(() => {
              ...
              return (
                <>
                        ...
                        <div key={r.id} style={{ 
                          ...
                          <div className="flex justify-between items-center" style={{ borderTop: '1px dashed #F0F4FF', paddingTop: 6 }}>
                            ...
                            <div className="text-right font-black" style={{ fontSize: 10, color: '#000000', lineHeight: 1.5 }}>
                              <div>{r.session_count} جلسة مقفلة 🔒</div>
                              <div style={{ fontSize: 9, color: '#000000', opacity: 0.6 }}>محفظة: {r.wallet_collected.toFixed(0)}ج · عمولة: {r.commission_amount.toFixed(0)}ج</div>
                            </div>
                          </div>
                          {r.notes && (
                  ...
                  {/* عداد صغير في الأسفل يوضح عدد المعروض من الإجمالي */}
                  <div className="text-center">
                    <span className="font-bold text-black" style={{ fontSize: 9, opacity: 0.7 }}>
                      عرض {Math.min(visibleSettlements, filtered.length)} من {filtered.length} تسوية
                    </span>
                  </div>
                </>
              ...
              <div key={g.id} style={{ background: '#fff', border: '2.5px solid #D0DCFF', borderRadius: 28, padding: 22, boxShadow: '0 4px 20px rgba(0,102,255,0.06)' }}>
                ...
                <div className="mb-3" style={{ background: '#FFF8F0', borderRadius: 14, padding: '10px 12px', border: '1.5px solid #FFD180' }}>
                  ...
                  {isEditingComm && (
                    <div className="flex items-center justify-center gap-2 mt-2">
                      <button onClick={() => setEditCommissionRate(r => Math.max(0, r - 1))} className="active:scale-90"
                        style={{ background: '#FF3333', color: '#fff', width: 30, height: 30, borderRadius: 9, display: 'flex', items: 'center', justify_center: 'center' }}>
                        <Minus size={14} />
                      </button>
                      <input type="number" value={editCommissionRate} onChange={e => setEditCommissionRate(Math.max(0, Math.min(100, parseInt(e.target.value) || 0)))}
                        className="bg-transparent text-center outline-none font-mono font-black"
                        style={{ width: 54, fontSize: 18, color: '#FF9500', background: '#fff', border: '1.5px solid #FFD180', borderRadius: 10, padding: '4px 0' }} />
                      <button onClick={() => setEditCommissionRate(r => Math.min(100, r + 1))} className="active:scale-90"
                        style={{ background: '#00CC66', color: '#fff', width: 30, height: 30, borderRadius: 9, display: 'flex', items: 'center', justify_center: 'center' }}>
                        <Plus size={14} />
                      </button>
                    </div>
                  )}

...
export default function AdminDashboard() {
  ...
  // تم إزالة setTick تماماً لمنع التحديث التلقائي الذي يسبب مشكلة انتهاء الجلسة
  ...
  // تحديث البيانات عند تحميل المكون أو تغيير الفلاتر فقط
  useEffect(() => {
    fetchDailyStats();
  }, [fetchDailyStats]);
    ...
    <div className="h-full bg-slate-950 text-white text-right p-5 overflow-y-auto pt-16">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
        ...
        <h2 className="text-xl font-black text-blue-400 flex items-center gap-2">
          لوحة المشرف العام <Shield size={20} />
        </h2>
      ...
      <div className="mb-20">
        ...
        <div className="bg-slate-900 p-5 rounded-[2rem] border border-slate-800 space-y-4">
          ...
          <div className="flex gap-2">
            <input className="flex-1 bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs font-bold text-right text-white outline-none" placeholder="اسم المستخدم" value={gUser} onChange={(e) => setGUser(e.target.value)} />
            <input className="flex-1 bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs font-bold text-right text-white outline-none" placeholder="رقم الهاتف" value={gPhone} onChange={(e) => setGPhone(e.target.value)} />
          </div>
          ...
          <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-3 text-white font-mono mb-3">
              <div>
                ...
                <input type="number" value={lat} onChange={(e) => setLat(parseFloat(e.target.value))} className="w-full bg-slate-900 p-2 rounded-lg border border-slate-800 text-xs outline-none" step="0.000001" />
              </div>
              <div>
                <span className="text-[8px] text-slate-500 block px-1">خط الطول</span>
                <input type="number" value={lng} onChange={(e) => setLng(parseFloat(e.target.value))} className="w-full bg-slate-900 p-2 rounded-lg border border-slate-800 text-xs outline-none" step="0.000001" />
              </div>
            </div>
            <button
              type="button"
              onClick={() => {
                if ('geolocation' in navigator) {
                  navigator.geolocation.getCurrentPosition(
                    (pos) => { setLat(pos.coords.latitude); setLng(pos.coords.longitude); toast.success('تم تحديث الموقع 📍'); },
                    () => toast.error('تعذر الحصول على الموقع')
                  );
                }
              }}
              className="w-full bg-blue-600/20 text-blue-400 py-3 rounded-xl text-xs font-black border border-blue-500/20 active:scale-95 transition-all"
            >
              📍 استخدم موقعي الحالي
            </button>
          </div>

...
import {
  ...
  MessageCircle, Send, Receipt, Search, HardHat,
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const garageReportFromStats=useMemo(()=>garages.map(g=>{
    ...
    return{name:g.name,garageId:g.id,count:gs.reduce((a,s)=>a+Number(s.total_sessions??0),0),revenue:gs.reduce((a,s)=>a+Number(s.confirmed_revenue??0),0),pendingRevenue:gs.reduce((a,s)=>a+Number(s.pending_revenue??0),0),cash:gs.reduce((a,s)=>a+Number(s.cash_revenue??0),0),instapay:gs.reduce((a,s)=>a+Number(s.instapay_revenue??0),0),wallet:gs.reduce((a,s)=>a+Number(s.wallet_revenue??0),0),cashwallet:gs.reduce((a,s)=>a+Number(s.cashwallet_revenue??0),0)};
  }),[garages,dailyStats]);

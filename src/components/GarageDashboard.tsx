        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {isFreeApplied && (
            <span className="font-black flex items-center gap-0.5 shrink-0 text-[8px] px-2 py-0.5 rounded" style={{ background: BRAND.greenLight, color: BRAND.greenDark, border: `1px solid ${BRAND.green}40` }}>
              <Gift size={10} /> {isFreeNow ? 'هدية ترحيبية نشطة 🎁' : 'انتهت الهدية'}
            </span>
          )}
        </div>
...
/* =========================================================================
   👑 المكون الرئيسي للوحة التحكم: GARAGE DASHBOARD (محمي ومطابق 100%)
   ========================================================================= */
export default function GarageDashboard() {
  ...
  // 1️⃣ جميع الـ State والـ Hooks في قمة الكومبوننت دائماً لمنع أي Crash
  ...
  const [editPaymentMode, setEditPaymentMode] = useState<'cash' | 'wallet' | 'both'>('both');
  ...
  // مزامنة حالة الجراج عند توفره
  useEffect(() => {
    if (garage) {
      setNewCarPrice(garage.basePrice || 15);
      setEditPrice(garage.basePrice || 15);
      setEditSpots(garage.availableSpots || 0);
      setEditCapacity(garage.capacity || 50);
      setEditPaymentMode((garage.payment_mode as any) || 'both');
  ...
  // 2️⃣ الآن الشروط الحارسة الآمنة (بعد انتهاء جميع الـ Hooks)
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center px-6" style={{ background: BRAND.bg, color: BRAND.navy }}>
        <div style={{ background: BRAND.card, borderRadius: 28, padding: 32, textAlign: 'center', maxWidth: 360, width: '100%', boxShadow: '0 4px 20px rgba(0,0,0,0.04)', border: `1px solid ${BRAND.border}` }}>
          ...
          <h2 className="font-black text-base" style={{ color: BRAND.navy, marginBottom: 8 }}>جاري فتح لوحة الجراج</h2>
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ background: BRAND.bg, color: BRAND.navy, padding: 16 }}>
      ...
      {ownerValetView && (
        ...
        >
          <span className="font-black text-white text-[9px] px-2.5 py-1 rounded" style={{ background: '#b45309' }}>
            الرجوع للمالك ↩️
          </span>
      ...
      {/* سجل العمليات مع تقرير التسوية واسم السايس المباشر */}
      <div className="mb-8">
        ...
        {isOwner && (
          <>
            ...
            {/* ⚖️ كارت تسوية الحساب المالي الدقيق مع التطبيق */}
            {filteredStats.totalCommission > 0 && (
              <div className="space-y-2 mb-3">
                <div className="grid grid-cols-2 gap-2">
                  <div className="text-center p-2 rounded-xl border bg-white" style={{ borderColor: BRAND.border }}>
                    <div className="text-[9px] font-bold text-slate-400">عمولة التطبيق</div>
                    <div className="font-black font-mono text-xs text-amber-600">{filteredStats.totalCommission.toFixed(0)} ج</div>
                  </div>
                  <div className="text-center p-2 rounded-xl border bg-white" style={{ borderColor: BRAND.border }}>
                    <div className="text-[9px] font-bold text-slate-400">صافي أرباحك</div>
                    <div className="font-black font-mono text-xs text-emerald-600">{filteredStats.totalNet.toFixed(0)} ج</div>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="p-3 border rounded-xl" style={{ background: isGarageOwed ? BRAND.greenLight : '#fff5f5', borderColor: isGarageOwed ? BRAND.green : '#fca5a5' }}>
                      <div className="flex justify-between items-center">
                        <div className="font-black font-mono text-base" style={{ color: isGarageOwed ? BRAND.greenDark : '#c53030' }}>
                          {absSettlement} ج.م
                        </div>
            ...
            return (
              <div key={session.id} className="p-2.5 border rounded-xl bg-white" style={{ borderColor: BRAND.border, opacity: isSettled ? 0.75 : 1 }}>
                ...
                <div className="flex justify-between items-center text-[9px] text-slate-400 font-bold border-t pt-1 mt-1" style={{ borderColor: BRAND.border }}>
                  <span>👤 بواسطة: <b style={{ color: BRAND.navy }}>{session.addedBy || 'المالك'}</b></span>
                  {time && <span className="font-mono">{time.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })}</span>}
                </div>
              </div>
            );
          })}

...
const vibrateDevice = () => {
  try {
    if ('vibrate' in navigator) navigator.vibrate([500, 150, 500, 150, 700]);
  } catch {}
};
...
export default function GarageDashboard() {
  ...
  /* ── Active sessions ── */
  /* ✅ تعديل: الجلسات النشطة تظهر للجميع (المالك والسايس)
     المالك يرى كل الجلسات
     السايس يرى كل الجلسات النشطة (مش بس بتاعته) عشان يعرف الجراج فيه إيه */
  ...
  /* ── الجلسات النشطة الخاصة بالسايس فقط (للإيراد) ── */
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async () => {
          await fetchAll();
          await fetchGarageDailyStatsRef.current();
        },
      )
      .on(
        'postgres_changes',
  ...
  useEffect(() => {
    fetchGarageDailyStatsRef.current = fetchGarageDailyStats;
  ...
  /* ✅ تعديل: فلترة العمليات المكتملة - السايس يرى فقط عملياته */
  ...
  const filteredStats = useMemo(() => {
    ...
    // ✅ السايس: نحسب من الجلسات المفلترة مباشرة (مش من daily_stats)
    const useDB = !isValet && garageDailyStats.length > 0;

    const cash = useDB
      ? garageDailyStats.reduce((a, s) => a + Number(s.cash_revenue ?? 0), 0)
      : c.filter(s => s.paymentMethod === 'cash').reduce((a, s) => a + getSessionRevenue(s), 0);
    const instapay = useDB
      ? garageDailyStats.reduce((a, s) => a + Number(s.instapay_revenue ?? 0), 0)
      : c.filter(s => s.paymentMethod === 'instapay').reduce((a, s) => a + getSessionRevenue(s), 0);
    const wallet = useDB
      ? garageDailyStats.reduce((a, s) => a + Number(s.wallet_revenue ?? 0), 0)
      : c.filter(s => s.paymentMethod === 'wallet').reduce((a, s) => a + getSessionRevenue(s), 0);
    const cashwallet = useDB
      ? garageDailyStats.reduce((a, s) => a + Number(s.cashwallet_revenue ?? 0), 0)
      : c.filter(s => s.paymentMethod === 'cashwallet').reduce((a, s) => a + getSessionRevenue(s), 0);
    const total = cash + instapay + wallet + cashwallet;
    const manual = c.filter(s => s.source === 'manual');
    const app = c.filter(s => s.source === 'app');
    const pr = u.reduce((a, s) => a + getSessionRevenue(s), 0);
  ...
  }, [filteredCompleted, getSessionRevenue, garageDailyStats, isValet]);

  /* ── Valet report (owner only) ── */
  const valetReport = useMemo(() => {
    ...
    return valetNames
      .map(v => {
        const valetSessions = filteredCompleted.filter(s => (s as any).addedBy === v.name);
  ...
  const getUndoRemainingSeconds = useCallback(
    (addedAt: number) => Math.max(0, UNDO_TIMEOUT_SECONDS - Math.floor((Date.now() - addedAt) / 1000)),
    [],
  );
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ background: '#EBF2FF', color: '#0A1628', padding: 16 }}>
          ...
          <div className="flex items-center gap-2 justify-end mt-1">
            ...
            >
              {isOwner ? (
                ...
                <>
                  <HardHat size={10} /> سايس {valetNumber} {currentValetName && `- ${currentValetName}`}
                </>
              )}
            </span>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-center flex-1">
                    <input
                      ...
                      value={editSpots}
                      onChange={e =>
                        setEditSpots(Math.max(0, Math.min(editCapacity, parseInt(e.target.value) || 0)))
                      }
                      className="bg-transparent text-center w-full outline-none font-mono font-black"
                ...
                {[
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span
                        className="font-bold"
                        style={{ fontSize: 10, color: v.name || v.pass ? v.color : '#CBD5E1' }}
                      >
                        {v.name || v.pass ? '✅ مفعّل' : '❌ غير مفعّل'}
                      </span>
                      <span className="font-black" style={{ fontSize: 12, color: '#0A1628' }}>
                        🅿️ سايس {v.n}
                      </span>
                    </div>
                    <input
      ...
      {confirmSession && (
        ...
        >
          ...
          >
            ...
            />
            <h3 className="font-black text-center mb-1" style={{ fontSize: 18, color: '#0A1628' }}>
              تأكيد تحصيل السداد
            </h3>
            <p className="text-center mb-5" style={{ fontSize: 12, color: '#7B8CA6' }}>
              لن يتم إنهاء الجلسة إلا بعد تأكيد السداد
            </p>

            <div
              className="mb-5"
              style={{ background: '#F0F4FF', borderRadius: 22, padding: 16, border: '2px solid #D0DCFF' }}
            >
              <div className="flex justify-between items-center mb-3">
                <span
                  ...
                  style={{
                    ...
                    padding: '4px 10px',
                    borderRadius: 12,
                    background: confirmSession.source === 'manual' ? '#FF9500' : '#0066FF',
                    color: '#fff',
                  }}
                >
                  {confirmSession.source === 'manual' ? 'يدوي' : 'تطبيق'}
                </span>
                <div className="font-black" style={{ fontSize: 18, color: '#0A1628' }}>
              ...
              {confirmSession.agreedPrice && confirmSession.agreedPrice !== garage.basePrice && (
                ...
                >
                  <p className="font-bold" style={{ fontSize: 11, color: '#E65100' }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="font-black font-mono" style={{ fontSize: 16, color: '#0A1628' }}>
            ...
            <div className="mb-5">
              ...
              {confirmSession.source === 'manual' ? (
                ...
                <div>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      >
                        ...
                        {(pm as any).disabled && (
                          <div className="font-bold" style={{ fontSize: 8, color: '#FF3333' }}>
                            🔒 غير متاح
                          </div>
                        )}
                      </button>
                  ...
                  >
                    <p className="font-bold" style={{ fontSize: 10, color: '#0066FF' }}>
      ...
      {/* ═══ Stats Cards ═══ */}
      {/* ✅ تعديل: السايس يرى إيراده اليومي فقط */}
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          {
            icon: <DollarSign size={22} />,
            value: isValet ? valetTodayRevenue.toFixed(0) : (garageDailyStats.length > 0 ? garageDailyStats.reduce((a, s) => a + Number(s.confirmed_revenue ?? 0), 0) : totalRevenue).toFixed(0),
          ...
          },
          {
            ...
            value: isValet ? valetActiveSessions.length : garage.basePrice,
            label: isValet ? 'جلساتي النشطة' : 'ج.م/ساعة',
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex-1 text-right">
                    <div className="flex items-center justify-end gap-2 mb-1">
                      <span className="font-black" style={{ fontSize: 14 }}>
                        🚗 {un.carPlate}
                      </span>
                      <span
                        className="font-bold"
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <span className="font-bold font-mono" style={{ fontSize: 11 }}>
                        ⏳ {rem} ثانية
                      </span>
                    </div>
                  </div>
      ...
      {carsOnTheWay.length > 0 && (
        <div className="mb-5">
          <h3
            ...
            style={{ fontSize: 15, color: '#0099DD' }}
          >
            <span
          ...
          <div className="space-y-3">
            {carsOnTheWay.map(car => {
              ...
              const started = sessions.some(
                s =>
                  s.carPlate.trim().toUpperCase() === car.carPlate.trim().toUpperCase() &&
                  s.status === 'active',
              );
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    borderRadius: 24,
                    padding: 18,
                    boxShadow: '0 4px 20px rgba(0,153,221,0.12)',
                  }}
                >
                  <div
                    className="absolute bottom-0 left-0 right-0"
                    style={{ height: 5, background: '#E0F7FA' }}
                  >
                    <div
                      className="h-full"
                      style={{
                        background: 'linear-gradient(90deg,#0099DD,#00CC66)',
                        borderRadius: 5,
                        width: `${Math.max(0, 100 - (rem / car.estimatedArrival) * 100)}%`,
                      }}
                    />
                  </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {started ? (
                        ...
                        <span
                          className="font-black"
                          style={{
                            background: rem <= 2 ? '#FF9500' : '#0099DD',
                            color: '#fff',
                            fontSize: 12,
                            padding: '6px 14px',
                            borderRadius: 14,
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span style={{ fontSize: 11, color: '#7B8CA6' }}>
            السعر:{' '}
            <span className="font-mono font-black" style={{ color: '#00AA44' }}>
              {garage.basePrice}ج
            </span>
          </span>
      ...
      {garageOffers.length > 0 && (
        <div className="mb-5">
          <h3
            className="font-black mb-3 flex items-center gap-2 justify-end"
            style={{ fontSize: 15, color: '#FF9500' }}
          >
            عروض أسعار ({garageOffers.length})
          </h3>
          <div className="space-y-3">
            {garageOffers.map(o => (
              <motion.div
              ...
              >
                <div className="flex justify-between items-center mb-3">
                  <div className="font-black font-mono" style={{ fontSize: 22, color: '#0A1628' }}>
                    {o.offeredPrice} ج.م
                    {o.offeredPrice < garage.basePrice && (
                      <span style={{ fontSize: 12, color: '#FF3333', marginRight: 8 }}>
                        (أقل من {garage.basePrice})
                      </span>
                    )}
                  </div>
                  <div className="font-black" style={{ fontSize: 15, color: '#0A1628' }}>
                    🚗 {o.carPlate}
                  </div>
                </div>
                <div className="flex gap-2">
      ...
      <div className="mb-5">
        {!showAddCar ? (
          <button
            ...
            style={{
              background:
                garage.availableSpots > 0 ? 'linear-gradient(135deg,#0066FF,#0044DD)' : '#D0DCFF',
                ...
                {[10, 15, 20, 25, 30].map(p => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      fontSize: 11,
                      background: newCarPrice === p ? '#0066FF' : '#F0F4FF',
                      color: newCarPrice === p ? '#fff' : '#64748b',
                      border: newCarPrice === p ? 'none' : '2px solid #D0DCFF',
                    }}
                  >
                    {p}
                  </button>
      ...
      {/* ═══ الجلسات النشطة ═══ */}
      {/* ✅ تظهر لكل من المالك والسايس - الكل يرى كل الجلسات النشطة */}
      <div className="mb-5">
        <h3
          className="font-black mb-3 flex items-center gap-2 justify-end"
          style={{ fontSize: 15, color: '#00AA44' }}
        ...
        <div className="space-y-3">
          {activeSessions.length === 0 ? (
            ...
            activeSessions.map(s => {
              ...
              const elapsedText = formatElapsed(el);
              const addedBy = (s as any).addedBy || '';

              /* ✅ تحديد إذا كانت الجلسة تخص السايس الحالي */
              const isMySession = !isValet || !currentValetNameLocal || addedBy === currentValetNameLocal;
              /* ✅ السايس يقدر يتحكم فقط في جلساته */
              const canControl = isOwner || isMySession;
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: `2.5px solid ${isValet && !isMySession ? '#E0E0E0' : isM ? '#FFD180' : '#D0DCFF'}`,
                    borderRadius: 24,
                    padding: 18,
                    opacity: isValet && !isMySession ? 0.7 : 1,
                  }}
                >
                  <div className="flex justify-between items-center mb-3">
                    <div className="flex items-center gap-2">
                  ...
                  {/* ✅ عرض اسم السايس اللي أضاف الجلسة */}
                  {addedBy && (
                    <div className="flex items-center justify-end gap-1 mb-2">
                      <span
                        ...
                        style={{
                          ...
                          padding: '3px 8px',
                          borderRadius: 10,
                          background: isMySession ? '#E8F5E9' : '#F5F5F5',
                          color: isMySession ? '#2E7D32' : '#757575',
                          border: `1px solid ${isMySession ? '#A5D6A7' : '#E0E0E0'}`,
                        }}
                      >
                        {isMySession && isValet ? '👤 جلستي' : `🅿️ ${addedBy}`}
                      </span>
                    </div>
                  )}
                  ...
                  {s.agreedPrice && s.agreedPrice !== garage.basePrice && (
                    ...
                    >
                      <span className="font-bold" style={{ fontSize: 10, color: '#E65100' }}>
                        سعر متفق: {s.agreedPrice} ج.م/ساعة
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      {canControl ? (
                        <button
                          onClick={() =>
                            openConfirmPayment(s.id, s.carPlate, cost, hrs, mins, s.source, s.agreedPrice)
                          }
                          className="font-black active:scale-95"
                          style={{
                            background: 'linear-gradient(135deg,#FF3333,#CC0000)',
      ...
      <div className="mb-8">
        ...
        {isOwner && (
          <div
            className="mb-4"
            style={{ background: '#fff', borderRadius: 24, padding: 16, border: '2px solid #D0DCFF' }}
          >
          ...
          <>
            ...
            >
              <div style={{ fontSize: 11, opacity: 0.8, marginBottom: 4 }}>
                {isValet
                  ? `إيرادي اليوم - ${currentValetName || 'سايس'} - ${formatLocalDateArabic(getLocalToday())}`
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[
                ...
                >
                  ...
                  <div className="font-bold" style={{ fontSize: 8, opacity: 0.8 }}>
                    {p.label}
                  </div>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              {[
                ...
                >
                  ...
                  <span className="font-black font-mono" style={{ fontSize: 16 }}>
                    {x.count}
                  </span>
                  <span className="font-black" style={{ fontSize: 12, marginRight: 4, color: '#ffffff' }}>
                    عربية
                  </span>
            ...
            {isOwner && valetReport.length > 0 && (
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-3 justify-end">
                  ...
                  <h4 className="font-black" style={{ fontSize: 13, color: '#334155' }}>
                    تقرير السياس
                  </h4>
                </div>
                  ...
                  {valetReport.map((v, i) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        >
                          <div
                            style={{ fontSize: 10, color: '#0066FF', fontWeight: 900, marginBottom: 2 }}
                          >
                            📱 تطبيق
                          </div>
                        ...
                        >
                          <div
                            style={{ fontSize: 10, color: '#FF9500', fontWeight: 900, marginBottom: 2 }}
                          >
                            ✋ يدوي
                          </div>
        ...
        <div className="space-y-2">
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span
                      ...
                      style={{ fontSize: 17, color: isM ? '#E65100' : '#0066FF' }}
                    >
                      {rev.toFixed(0)} ج.م
                    </span>
                    <span
                  ...
                  <div className="flex items-center gap-1">
                    <span
                      className="rounded-full"
                      style={{ width: 6, height: 6, background: isM ? '#FF9500' : '#0066FF' }}
                    />
                    <span className="font-black" style={{ fontSize: 15, color: '#0A1628' }}>
                      {session.carPlate}
                    </span>
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-1.5">
                    ...
                    {session.agreedPrice && session.agreedPrice !== garage.basePrice && (
                      <span className="font-bold" style={{ fontSize: 9, color: '#E65100' }}>
          ...
          {filteredCompleted.length === 0 && (
            <div
              className="text-center"
              style={{ background: '#fff', borderRadius: 24, padding: 32, border: '2px solid #D0DCFF' }}
            >
              ...
              <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>
                {isValet ? 'العمليات المعروضة هي عملياتك فقط' : 'جرب تغيير التاريخ'}
              </p>
            </div>
          )}
        </div>

...
const getAudioCtx = (): AudioContext | null => {
  ...
  if (audioCtxInstance.state === 'closed') { audioCtxInstance = null; audioCtxReady = false; return null; }
  return audioCtxInstance;
};
...
const sendNotification = (title: string, body: string, tag: string) => {
  try {
    if ('Notification' in window && Notification.permission === 'granted') {
      const n = new Notification(title, { body, icon: '/icons/icon-192x192.png', tag, requireInteraction: true, silent: false });
...
const playFirstAlert = async () => {
  let ctx = getAudioCtx(); if (!ctx || !audioCtxReady) ctx = await initAudioContext(); if (!ctx) return;
  try {
    if (ctx.state === 'suspended') await ctx.resume();
    [{ freq: 800, delay: 0, dur: 0.15 }, { freq: 1000, delay: 0.2, dur: 0.15 }, { freq: 1200, delay: 0.4, dur: 0.2 }, { freq: 1400, delay: 1.5, dur: 0.4 }].forEach(({ freq, delay, dur }) => {
      ...
      g.gain.setValueAtTime(0.5, ctx!.currentTime + delay); g.gain.exponentialRampToValueAtTime(0.01, ctx!.currentTime + delay + dur);
...
const fireNewCarAlert = (carPlate: string, customerName?: string, agreedPrice?: number) => {
  ...
  sendNotification('🚨 سيارة في الطريق!', [`🚗 ${carPlate}`, customerName ? `👤 ${customerName}` : '', agreedPrice ? `💰 ${agreedPrice} ج.م/ساعة` : ''].filter(Boolean).join('\n'), `incoming-${carPlate}`);
};
...
const playApproachingAlert = async () => {
  ...
  try {
    if (ctx.state === 'suspended') await ctx.resume();
    [{ freq: 1000, delay: 0, dur: 0.2 }, { freq: 1300, delay: 0.25, dur: 0.2 }, { freq: 1600, delay: 0.5, dur: 0.3 }, { freq: 1800, delay: 1.8, dur: 0.5 }].forEach(({ freq, delay, dur }) => {
      const o = ctx!.createOscillator(); const g = ctx!.createGain(); o.connect(g); g.connect(ctx!.destination); o.type = 'square'; o.frequency.value = freq;
      g.gain.setValueAtTime(0.6, ctx!.currentTime + delay); g.gain.exponentialRampToValueAtTime(0.01, ctx!.currentTime + delay + dur);
      o.start(ctx!.currentTime + delay); o.stop(ctx!.currentTime + delay + dur + 0.05);
    });
  } catch (e) { console.warn('⚠️', e); }
};
...
export default function GarageDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => { try { const s = await subscribeToPush(currentGarageId); if (s) pushSubscribedGarageRef.current = currentGarageId; } catch (e) { console.error('❌', e); } })();
  ...
  useEffect(() => {
    ...
    prevIncomingIdsRef.current.forEach(id => { if (!ids.has(id)) { approachAlertedRef.current.delete(id); try { if ('vibrate' in navigator) navigator.vibrate(0); } catch {} } });
  ...
  useEffect(() => {
    carsOnTheWay.forEach(car => {
      ...
      const s = toMs(car.startTime); const el = (Date.now() - s) / 60000;
  ...
  const getSessionRevenue = useCallback((s: any) => {
    ...
    if (s.endTime && s.startTime) { const st = toMs(s.startTime); const en = toMs(s.endTime); return calculateCost(Math.max(0, Math.floor((en - st) / 1000)), Number(s.agreedPrice ?? garage?.basePrice ?? 0)); }
    return 0;
  }, [garage?.basePrice]);
  ...
  const valetTodayRevenue = useMemo(() => {
    ...
    return completedSessions
      .filter(s => s.revenueConfirmed && (s as any).addedBy === currentValetNameLocal && s.endTime && timestampToLocalDate(toMs(s.endTime)) === getLocalToday())
  ...
  const totalRevenue = useMemo(() => {
    ...
    return garageDailyStats.reduce((a, s) => a + Number(s.confirmed_revenue ?? 0), 0) ||
  ...
  const getActiveCost = useCallback((s: any) => {
    const st = toMs(s.startTime); const el = Math.max(0, Math.floor((Date.now() - st) / 1000));
  ...
  const valetReport = useMemo(() => {
    ...
    return [
      ...
      return { ...v, count: vs.length, appCount: ac.length, manualCount: mc.length,
  ...
  const handleUndoSession = useCallback((un: UndoableSession) => {
    if (!garage) return;
    removeSession(un.sessionId); if (un.localId !== un.sessionId) removeSession(un.localId);
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const sd = useStore.getState().sessions.find(s => s.id === sc.id); const ia = sd?.source === 'app';
      setConfirmSession(null); setUndoableSessions(p => p.filter(u => u.sessionId !== sc.id && u.localId !== sc.id));
...
const handleCarArrived = async (car: any) => {
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ background: '#EBF2FF', color: '#0A1628', padding: 16 }}>
            ...
            <div className="mb-6">
              ...
              <div style={{ background: '#F0F4FF', borderRadius: 22, padding: 16, border: '2px solid #D0DCFF' }}>
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-center flex-1"><input type="number" value={editCapacity} onChange={e => setEditCapacity(Math.max(editSpots, parseInt(e.target.value) || editSpots))} className="bg-transparent text-center w-full outline-none font-mono font-black" style={{ fontSize: 28, color: '#7C3AED' }} /><div className="font-bold" style={{ fontSize: 11, color: '#94a3b8' }}>مكان إجمالي</div></div>
            ...
            <div className="mb-6">
              ...
              <div style={{ background: '#F0F4FF', borderRadius: 22, padding: 16, border: '2px solid #D0DCFF' }}>
                {[{ n: 1, name: editValet1Name, setName: setEditValet1Name, pass: editValet1Pass, setPass: setEditValet1Pass, color: '#0066FF' },
                  ...
                  { n: 3, name: editValet3Name, setName: setEditValet3Name, pass: editValet3Pass, setPass: setEditValet3Pass, color: '#FF8800' }].map((v, i) => (
                  <div key={i} className={i < 2 ? 'mb-4 pb-4' : ''} style={i < 2 ? { borderBottom: '1px solid #D0DCFF' } : {}}>
                    <div className="flex items-center justify-between mb-2"><span className="font-bold" style={{ fontSize: 10, color: v.name || v.pass ? v.color : '#CBD5E1' }}>{v.name || v.pass ? '✅ مفعّل' : '❌ غير مفعّل'}</span><span className="font-black" style={{ fontSize: 12 }}>🅿️ سايس {v.n}</span></div>
          ...
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} transition={{ type: 'spring', damping: 25 }} className="w-full max-w-sm" style={{ background: '#fff', borderRadius: '32px 32px 20px 20px', padding: 24 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-5">
              ...
              {confirmSession.source === 'manual' ? (
                <div className="text-center" style={{ background: 'linear-gradient(135deg,#00CC66,#00AA55)', borderRadius: 18, padding: 18, color: '#fff' }}><div style={{ fontSize: 28 }}>💵</div><div className="font-black" style={{ fontSize: 15 }}>نقدي</div></div>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  {[{ id: 'cash', label: 'نقدي', icon: '💵', bg: '#00CC66' }, { id: 'instapay', label: 'إنستاباي', icon: '📱', bg: '#7C3AED' }, { id: 'wallet', label: 'المحفظة', icon: '👝', bg: '#0066FF', disabled: true }, { id: 'cashwallet', label: 'محفظة كاش', icon: '📲', bg: '#FF8800' }].map(pm => (
                    <button key={pm.id} onClick={() => !(pm as any).disabled && setConfirmPaymentMethod(pm.id)} disabled={(pm as any).disabled} className="text-center active:scale-95" style={{ borderRadius: 18, padding: 14, background: (pm as any).disabled ? '#F0F4FF' : confirmPaymentMethod === pm.id ? pm.bg : '#fff', color: (pm as any).disabled ? '#94a3b8' : confirmPaymentMethod === pm.id ? '#fff' : '#475569', border: (pm as any).disabled ? '2px solid #D0DCFF' : confirmPaymentMethod === pm.id ? 'none' : '2px solid #D0DCFF' }}>
                      <div style={{ fontSize: 24 }}>{pm.icon}</div><div className="font-black" style={{ fontSize: 11 }}>{pm.label}</div>
                    </button>
                  ))}
                </div>
              )}
      ...
      {/* ═══════════════════════════════════════════
          ██  السايس فقط: سيارات في الطريق + إضافة + جلسات نشطة + عروض
          ═══════════════════════════════════════════ */}
      {isValet && (
        <>
          {/* ── Undo Banners ── */}
                  ...
                  return (
                    <motion.div key={car.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} style={{ background: '#fff', border: '2.5px solid #00BBE0', borderRadius: 24, padding: 18 }}>
                      ...
                      <div className="mb-3 space-y-2" style={{ background: '#F0F4FF', borderRadius: 18, padding: 14, border: '1px solid #D0DCFF' }}>
                        ...
                        <div className="flex items-center justify-between"><span className="font-black font-mono" style={{ fontSize: 15, color: '#00AA44' }}>{car.agreedPrice} ج.م / ساعة</span><div className="flex items-center gap-1" style={{ color: '#94a3b8' }}><DollarSign size={14} /><span className="font-bold" style={{ fontSize: 11 }}>السعر</span></div></div>
                      </div>
  ...
  || addedBy === `سايس ${valetNumber}`;
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        {isMySession ? (
                          ...
                          <span className="font-bold" style={{ fontSize: 10, color: '#94a3b8', padding: '8px 14px', background: '#F5F5F5', borderRadius: 14 }}>🔒 جلسة سايس آخر</span>
                        )}
      ...
      <div className="mb-8">
        ...
        {filteredCompleted.length > 0 && (
          <>
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[{ label: 'نقدي', value: filteredStats.cash, icon: '💵', bg: '#00CC66' }, { label: 'إنستاباي', value: filteredStats.instapay, icon: '📱', bg: '#7C3AED' }, { label: 'محفظة', value: filteredStats.wallet, icon: '👝', bg: '#0066FF' }, { label: 'كاش', value: filteredStats.cashwallet, icon: '📲', bg: '#FF8800' }].map(p => (
                <div key={p.label} className="text-center" style={{ background: p.bg, borderRadius: 18, padding: '12px 6px', color: '#fff' }}><div style={{ fontSize: 20, marginBottom: 2 }}>{p.icon}</div><div className="font-black font-mono" style={{ fontSize: 15 }}>{p.value.toFixed(0)}</div><div className="font-bold" style={{ fontSize: 8, opacity: 0.8 }}>{p.label}</div></div>
              ))}
            </div>

            <div className="grid grid-cols-2 gap-2 mb-4">
              {[{ label: 'يدوي', count: filteredStats.manualCount, total: filteredStats.manualTotal, bg: '#FF9500' }, { label: 'تطبيق', count: filteredStats.appCount, total: filteredStats.appTotal, bg: '#0066FF' }].map(x => (
                <div key={x.label} className="text-center" style={{ background: x.bg, borderRadius: 18, padding: 14, color: '#fff' }}><div className="font-black" style={{ fontSize: 12, marginBottom: 4 }}>{x.label}</div><span className="font-black font-mono" style={{ fontSize: 16 }}>{x.count}</span><span className="font-black" style={{ fontSize: 12, marginRight: 4 }}> عربية</span><div style={{ fontSize: 10, opacity: 0.8 }}>({x.total.toFixed(0)} ج.م)</div></div>
              ))}
            </div>

            {/* تقرير السياس - المالك فقط */}
        ...
        <div className="space-y-2">
            ...
            return (
              <div key={session.id} style={{ background: isC ? (isM ? '#FFF8F0' : '#EBF5FF') : '#FFF8F0', border: `2px solid ${isC ? (isM ? '#FFD180' : '#A0C4FF') : '#FFD180'}`, borderRadius: 18, padding: 14 }}>
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center gap-1"><span className="rounded-full" style={{ width: 6, height: 6, background: isM ? '#FF9500' : '#0066FF' }} /><span className="font-black" style={{ fontSize: 15 }}>{session.carPlate}</span></div>
                </div>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-1.5">
                    {session.paymentMethod && <span className="font-bold" style={{ fontSize: 9, padding: '3px 10px', borderRadius: 10, color: '#fff', background: session.paymentMethod === 'cash' ? '#00CC66' : session.paymentMethod === 'instapay' ? '#7C3AED' : session.paymentMethod === 'wallet' ? '#0066FF' : '#FF8800' }}>{session.paymentMethod === 'cash' ? '💵 نقدي' : session.paymentMethod === 'instapay' ? '📱 إنستاباي' : session.paymentMethod === 'wallet' ? '👝 محفظة' : '📲 كاش'}</span>}
                  </div>
          ...
          {filteredCompleted.length === 0 && (
            <div className="text-center" style={{ background: '#fff', borderRadius: 24, padding: 32, border: '2px solid #D0DCFF' }}><div style={{ fontSize: 36, marginBottom: 12 }}>📭</div><p className="font-bold" style={{ fontSize: 14, color: '#7B8CA6' }}>{isValet ? 'لا توجد عمليات لك اليوم' : 'لا توجد عمليات'}</p></div>
          )}
        </div>
      </div>
    </div>

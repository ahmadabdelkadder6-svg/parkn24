  ...
  return (
    <div className="h-full overflow-y-auto" style={{ background: '#EBF2FF', color: '#0A1628', padding: 16 }}>
      ...
      <div className={`grid ${isOwner ? 'grid-cols-3' : 'grid-cols-2'} gap-3 mb-5`}>
        {isOwner ? (
          <>
            <div className="text-center" style={{ background: 'linear-gradient(135deg,#00CC66,#00AA55)', borderRadius: 22, padding: '18px 10px', color: '#fff', boxShadow: '0 6px 24px rgba(0,204,102,0.3)' }}>
              <div className="mx-auto mb-1" style={{ opacity: 0.9 }}><span className="font-black" style={{ fontSize: 14 }}>ج.م</span></div>
              <div className="font-black font-mono" style={{ fontSize: 24 }}>{topCardConfirmedRevenue.toFixed(0)}</div>
              <div className="font-bold" style={{ fontSize: 9, opacity: 0.8 }}>مؤكد</div>
            </div>
            <div onClick={openSettings} className="text-center cursor-pointer active:scale-95" style={{ background: 'linear-gradient(135deg,#0066FF,#0044DD)', borderRadius: 22, padding: '18px 10px', color: '#fff', boxShadow: '0 6px 24px rgba(0,102,255,0.3)' }}>
              ...
              <div className="font-bold flex items-center justify-center gap-1" style={{ fontSize: 9, opacity: 0.8 }}>شاغر <Edit3 size={9} /></div>
            </div>
            <div onClick={openSettings} className="text-center cursor-pointer active:scale-95" style={{ background: 'linear-gradient(135deg,#7C3AED,#5B21B6)', borderRadius: 22, padding: '18px 10px', color: '#fff', boxShadow: '0 6px 24px rgba(124,58,237,0.3)' }}>
              <div className="mx-auto mb-1" style={{ opacity: 0.9 }}><span className="font-black" style={{ fontSize: 14 }}>ج.م</span></div>
              <div className="font-black font-mono" style={{ fontSize: 24 }}>{garage.basePrice}</div>
              <div className="font-bold flex items-center justify-center gap-1" style={{ fontSize: 9, opacity: 0.8 }}>سعر/ساعة <Edit3 size={9} /></div>
            </div>
          </>
        ) : (
          <>
            <div className="text-center" style={{ background: 'linear-gradient(135deg,#0066FF,#0044DD)', borderRadius: 22, padding: '18px 10px', color: '#fff', boxShadow: '0 6px 24px rgba(0,102,255,0.3)' }}>
              <div className="mx-auto mb-1" style={{ opacity: 0.9 }}><Car size={22} /></div>
              ...
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3" style={{ background: '#fff', border: '2.5px solid #0066FF', borderRadius: 24, padding: 18 }}>
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 mt-2 justify-end">
                    {[10, 15, 20, 25, 30].map(p => (<button key={p} onClick={() => setNewCarPrice(p)} className="font-black active:scale-95" style={{ padding: '5px 12px', borderRadius: 10, fontSize: 11, background: newCarPrice === p ? '#0066FF' : '#F0F4FF', color: newCarPrice === p ? '#fff' : '#64748b', border: newCarPrice === p ? 'none' : '2px solid #D0DCFF' }}>{p}</button>))}
                  </div>
                </div>
                <div className="flex gap-2">
      ...
      {isValet && (
        <div className="mb-5" style={{ background: '#fff', borderRadius: 20, padding: '12px 16px', border: '2px solid #D0DCFF' }}>
          <div className="flex items-center justify-between">
            <span className="font-black flex items-center gap-1" style={{ fontSize: 14, color: '#0A1628' }}>
            ...
            <div className="flex items-center gap-4">
              ...
              <div style={{ width: 2, height: 28, background: '#D0DCFF', borderRadius: 2 }} />
              <button onClick={() => setValetEditSpots(!valetEditSpots)} className="active:scale-95" style={{ fontSize: 11, color: valetEditSpots ? '#0066FF' : '#7B8CA6', background: 'transparent' }}><Edit3 size={12} style={{ color: '#0066FF' }} /></button>
            </div>
          </div>
          {valetEditSpots && (
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <span className="font-bold" style={{ fontSize: 13, background: '#fff', padding: '8px 16px', borderRadius: 14, border: '2px solid #D0DCFF', color: '#7B8CA6' }}>{filteredCompleted.length} عملية</span>
          <h3 className="font-black flex items-center gap-2" style={{ fontSize: 18, color: '#334155' }}>{isValet ? 'سجل عملياتي اليوم' : 'سجل العمليات'} <FileText size={18} /></h3>
        </div>
        ...
        {isValet && (
          <div className="mb-4 text-center" style={{ background: '#EBF2FF', borderRadius: 18, padding: '10px 16px', border: '2px solid #D0DCFF' }}>
            <span className="font-black" style={{ fontSize: 14, color: '#0066FF' }}>📅 {formatLocalDateArabic(getLocalToday())} - عملياتي فقط</span>
          </div>
        )}

        {isOwner && selectedValetFilter && (
          <div className="flex items-center justify-between mb-3" style={{ background: '#EBF2FF', borderRadius: 14, padding: '10px 14px', border: '1.5px solid #D0DCFF' }}>
            <button onClick={() => setSelectedValetFilter(null)} className="font-bold active:scale-95" style={{ fontSize: 12, color: '#FF3333' }}>✕ إلغاء الفلتر</button>
            <span className="font-black" style={{ fontSize: 13, color: '#0066FF' }}>🅿️ عمليات: {selectedValetFilter}</span>
          </div>
        )}
        ...
        {filteredCompleted.length > 0 && (
          <>
            {filteredStats.pendingCount > 0 && (
              <div className="mb-4" style={{ background: 'linear-gradient(135deg,#FF9500,#FF7700)', borderRadius: 22, padding: 20, color: '#fff', boxShadow: '0 8px 28px rgba(255,149,0,0.3)' }}>
                <div className="flex justify-between items-center">
                  <div className="text-right flex-1">
                    <h3 className="font-black mb-1" style={{ fontSize: 17 }}>⏳ عمليات معلقة للتأكيد</h3>
                    <span className="font-black" style={{ fontSize: 13, background: 'rgba(255,255,255,0.2)', padding: '4px 12px', borderRadius: 12 }}>{filteredStats.pendingCount} عملية</span>
                  </div>
                  {isOwner && (
                    <div className="text-left mr-4">
                      <div className="font-black font-mono" style={{ fontSize: 32 }}>{filteredStats.pendingRevenue.toFixed(0)}</div>
                      <div className="font-bold text-center" style={{ fontSize: 12, opacity: 0.85 }}>ج.م</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            {isOwner && (
              <>
                <div className="mb-4 text-center" style={{ background: 'linear-gradient(135deg,#00CC66,#00AA55)', borderRadius: 24, padding: 24, color: '#fff' }}>
                  <div style={{ fontSize: 12, opacity: 0.8, marginBottom: 4 }}>
                    {`مؤكد - ${logDateFrom === logDateTo ? formatLocalDateArabic(logDateFrom) : `${logDateFrom} → ${logDateTo}`}`}
                  </div>
                  <div className="font-black font-mono" style={{ fontSize: 44 }}>{filteredStats.total.toFixed(0)} ج.م</div>
                  <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>{filteredCompleted.filter(s => s.revenueConfirmed).length} عملية مؤكدة</div>
                </div>
                ...
                {filteredStats.totalCommission > 0 && (
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    ...
                    <div className="text-center" style={{ background: 'linear-gradient(135deg,#00AA55,#008844)', borderRadius: 18, padding: 16, color: '#fff' }}>
                      ...
                      <div className="font-bold" style={{ fontSize: 10, opacity: 0.8 }}>صافي الإيراد</div>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  <div className="text-center" style={{ background: '#00CC66', borderRadius: 18, padding: '14px 6px', color: '#fff' }}>
                    ...
                    <div className="font-black font-mono" style={{ fontSize: 20 }}>{filteredStats.cash.toFixed(0)}</div>
                    <div className="font-bold" style={{ fontSize: 10, opacity: 0.8 }}>نقدي</div>
                  </div>
                  <div className="text-center" style={{ background: '#0066FF', borderRadius: 18, padding: '14px 6px', color: '#fff' }}>
                    <div style={{ fontSize: 22, marginBottom: 2 }}>👝</div>
                    <div className="font-black font-mono" style={{ fontSize: 20 }}>{filteredStats.wallet.toFixed(0)}</div>
                    <div className="font-bold" style={{ fontSize: 10, opacity: 0.8 }}>محفظة</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  {[
                    ...
                    <div key={x.label} className="text-center" style={{ background: x.bg, borderRadius: 18, padding: 16, color: '#fff' }}>
                      <div className="font-black" style={{ fontSize: 14, marginBottom: 4 }}>{x.label}</div>
                      <span className="font-black font-mono" style={{ fontSize: 20 }}>{x.count}</span>
                      <span className="font-black" style={{ fontSize: 14, marginRight: 4 }}> عربية</span>
                      <div style={{ fontSize: 12, opacity: 0.8 }}>({x.total.toFixed(0)} ج.م)</div>
                    </div>
                  ))}
                </div>
            ...
            {isValet && (
              <div className="mb-4 text-center" style={{ background: '#fff', borderRadius: 20, padding: 18, border: '2px solid #D0DCFF' }}>
                ...
                <div className="font-bold" style={{ fontSize: 13, color: '#94a3b8' }}>عملية مكتملة</div>
              </div>
            )}
          </>
        )}

        {/* قائمة العمليات المكتملة */}
        <div className="space-y-3">
          {filteredCompleted.map(session => {
            ...
            return (
              <div key={session.id} style={{ background: isC ? (isM ? '#FFF8F0' : '#EBF5FF') : '#FFFBF0', border: `2.5px solid ${isC ? (isM ? '#FFD180' : '#A0C4FF') : '#FFD180'}`, borderRadius: 22, padding: 18 }}>
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    {(isOwner || !isC) && (
                      <span className="font-mono font-black" style={{ fontSize: 26, color: isM ? '#E65100' : '#0066FF' }}>{rev.toFixed(0)} ج.م</span>
                    )}
                    <span className="font-black" style={{ fontSize: 13, padding: '6px 14px', borderRadius: 14, background: isM ? '#FF9500' : '#0066FF', color: '#fff' }}>{isM ? 'يدوي' : 'تطبيق'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="rounded-full" style={{ width: 8, height: 8, background: isM ? '#FF9500' : '#0066FF' }} />
                    <span className="font-black" style={{ fontSize: 22 }}>{session.carPlate}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  {addedBy && isOwner && (
                    <span className="font-bold cursor-pointer active:scale-95" onClick={() => setSelectedValetFilter(selectedValetFilter === addedBy ? null : addedBy)}
                      style={{ fontSize: 11, padding: '5px 12px', borderRadius: 12, background: selectedValetFilter === addedBy ? '#0066FF' : '#EBF2FF', color: selectedValetFilter === addedBy ? '#fff' : '#0066FF', border: '1.5px solid #D0DCFF' }}>
                      🅿️ {addedBy}
                    </span>
                  )}
                  {!isC ? (
                    <button onClick={async () => { await confirmRevenue(session.id); await fetchGarageDailyStats(); toast.success('تأكيد ✅'); }} className="font-black active:scale-95" style={{ background: '#FF9500', color: '#fff', padding: '5px 14px', borderRadius: 12, fontSize: 12 }}>⏳ تأكيد</button>
                  ) : (
                    <span className="font-black" style={{ fontSize: 12, color: '#00AA44' }}>✅ مؤكد</span>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    {session.paymentMethod && (
                      <span className="font-black" style={{ fontSize: 12, padding: '5px 14px', borderRadius: 12, color: '#fff', background: session.paymentMethod === 'cash' ? '#00CC66' : session.paymentMethod === 'instapay' ? '#7C3AED' : session.paymentMethod === 'wallet' ? '#0066FF' : '#FF8800' }}>
                        {session.paymentMethod === 'cash' ? '💵 نقدي' : session.paymentMethod === 'instapay' ? '📱 إنستاباي' : session.paymentMethod === 'wallet' ? '👝 محفظة' : '📲 كاش'}
                      </span>
                    )}
                  </div>
                  {time && <span className="font-mono font-black" style={{ fontSize: 15, color: '#475569' }}>{time.toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' })} · {time.toLocaleDateString('ar-EG', { month: 'short', day: 'numeric' })}</span>}
                </div>
              </div>
            );
          })}
          {filteredCompleted.length === 0 && (
            <div className="text-center" style={{ background: '#fff', borderRadius: 24, padding: 36, border: '2px solid #D0DCFF' }}>
              <div style={{ fontSize: 44, marginBottom: 14 }}>📭</div>
              <p className="font-black" style={{ fontSize: 16, color: '#7B8CA6' }}>{isValet ? 'لا توجد عمليات لك اليوم' : 'لا توجد عمليات'}</p>
            </div>
          )}
        </div>
      </div>

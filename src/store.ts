...
// ✅ mapSession محسّن - تحويل دقيق للوقت
const mapSession = (r: any): ParkingSession => {
  ...
  if (rawEnd) {
    if (typeof rawEnd === 'string') {
      ...
      endTime = rawEnd < 1_000_000_000_000 ? rawEnd * 1000 : rawEnd;
    }
    if (endTime && (!Number.isFinite(endTime) || endTime <= 0)) {
      endTime = undefined;
    }
  }
...
const resolveStartTime = (rawStartTime: any): number => {
  ...
  if (!Number.isFinite(result) || result <= 0 || result > nowMs + 60000) {
    result = nowMs;
  }
  return result;
};
...
export const useStore = create<AppState>((set, get) => ({
  ...
  fetchAll: async () => {
    ...
    const localOnlySessions = currentSessions.filter((cs) =>
      !supabaseSessionIds.has(cs.id) && cs.status === 'active' && !supabaseActiveKeys.has(`${normalizePlate(cs.carPlate)}::${cs.source}`) && !deletedSessionIds.has(cs.id) && Date.now() - cs.startTime < 10000
    );
    ...
    const mergedTopUps = supabaseTopUps.map((st) => { const lv = currentTopUps.find((ct) => ct.id === st.id); if (lv && lv.status !== 'pending' && st.status === 'pending') return lv; return st; });
    ...
    const mergedMessages = supabaseMessages.map((sm) => {
      ...
      if (lv) { if (lv.status !== 'pending' && sm.status === 'pending') return lv; if (sm.status !== 'pending' && lv.status === 'pending') return sm; const smT = sm.repliedAt ?? sm.timestamp; const lvT = lv.repliedAt ?? lv.timestamp; if (smT > lvT) return sm; return lv; }
      return sm;
    });
  ...
  updateGarage: (id, updates) => {
    ...
    updateGarageTimeout = setTimeout(async () => {
      for (const [garageId, dbUpdates] of pendingGarageUpdates.entries()) { await supabase.from('garages').update(dbUpdates).eq('id', garageId); }
      pendingGarageUpdates.clear(); updateGarageTimeout = null;
    }, 500);
  },

  adjustGarageSpots: async (id, delta) => {
    set((st) => ({ garages: st.garages.map((g) => { if (g.id !== id) return g; return { ...g, availableSpots: Math.max(0, Math.min(g.capacity, g.availableSpots + delta)) }; }) }));
    if (!isSupabaseConfigured()) return;
    try {
  ...
  addSession: async (s) => {
    ...
    try {
      const existingLocal = get().sessions.find((existing) => samePlate(existing.carPlate, normalizedPlate) && existing.status === 'active' && existing.source === s.source);
      if (existingLocal) return existingLocal.id;

      if (isSupabaseConfigured()) {
        try {
          const { data: dbCheck } = await supabase.from('sessions').select('id, car_plate, source').eq('status', 'active').eq('car_plate', normalizedPlate).eq('source', s.source).limit(1);
  ...
  endSession: async (id, totalPrice, paymentMethod) => {
    ...
    try {
      ...
      const { error } = await supabase.from('sessions').update({
        end_time: new Date(now).toISOString(), total_price: safeTotalPrice,
  ...
  removeSession: async (id) => {
    ...
    if (target) {
      state.sessions.forEach((s) => {
        if (samePlate(s.carPlate, target.carPlate) && s.source === 'manual' && s.status === 'active' && Math.abs(s.startTime - target.startTime) < 10000) { idsToDelete.add(s.id); deletedSessionIds.add(s.id); }
      });
    }
  ...
  approveTopUp: async (id) => {
    ...
    const realUserId = dbRow.user_id || topUp.userId || ''; const realUserPhone = dbRow.user_phone || topUp.userPhone || '';
    ...
    if (currentUser && (currentUser.phone === userData.phone || (currentUser as any).id === userData.id)) {
      const updated = { ...currentUser, wallet: newWallet }; set({ currentUser: updated }); safeSetStorage('currentUser', updated);
    }
  },
  ...
  removeIncomingCar: async (id) => {
    ...
    if (!isSupabaseConfigured()) return;
    try { await supabase.from('incoming_cars').delete().eq('id', id); if (savedCarPlate && savedGarageId) { await supabase.from('incoming_cars').delete().eq('car_plate', savedCarPlate).eq('garage_id', savedGarageId); } } catch (err) { console.error('❌', err); }
  ...
  addMessage: async (msg) => {
    ...
    try {
      ...
      return { success: true };
    } catch (err) { console.error('❌', err); set((st) => ({ messages: (st.messages ?? []).filter((m) => m.id !== optimisticMessage.id) })); return { success: false, error: err instanceof Error ? err.message : 'حدث خطأ غير متوقع' }; }
  },
...
export function setupRealtime() {
  ...
  channel.subscribe((status) => {
    if (status === 'SUBSCRIBED') { console.log('✅ Realtime connected:', channelName); if (pollingInterval) clearInterval(pollingInterval); pollingInterval = setInterval(() => { if (!isOperationInProgress) useStore.getState().fetchAll(); }, 10000); }
  ...
  window.addEventListener('beforeunload', () => { if (refreshTimeout) clearTimeout(refreshTimeout); if (pollingInterval) clearInterval(pollingInterval); if (pauseTimeout) clearTimeout(pauseTimeout); channel.unsubscribe(); supabase.removeChannel(channel); });
}